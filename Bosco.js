...
module.exports = bosco = async (bosco, mek) => {
	try {
		...
		const catl = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 400, "message": teks, "footerText": "*ᴢᴇᴅ ʙᴏᴛ*", "thumbnail": dfrply, "surface": 'CATALOG' }}, {quoted:ftrol})
...
// AUTO
      ...
      const sendWebp = async(to, url) => {
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           bosco.sendMessage(to, media, type, {quoted: mek, "externalAdReply": { "title": `${' '}𝐙𝐄𝐃 𝐁𝐎𝐓${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      //FUNCTION
            ...
            bosco.sendMessage(mek.key.remoteJid,`@${owner} *Currently Offline!*\n\n*Reason :* *${alasan}*\n*Since :* *${heheh.hours} 'O'clock*, *${heheh.minutes}* *Minute*, *${heheh.seconds}* *Seconds ago*\n\n *Please contact again later...*`, MessageType.text,{contextInfo:{ mentionedJid: [`${owner}@s.whatsapp.net`],'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': "0@s.whatsapp.net", 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": "*𝐙𝐄𝐃 𝐈𝐒 OFFLINE*", 'jpegThumbnail': fs.readFileSync('./ds.jpg')}}}})
            }
            }   
        if (mek.key.remoteJid.endsWith('@g.us') && offline) {
        if (!mek.key.fromMe){
      ...
      const sendFileFromUrl = async(link, type, options) => {
          ...
          if (!packname) packname = '!ᴢᴇᴅ ʙᴏᴛ '; if (!author) author = '(ɪᴍᴀꜱʜ)';author = author.replace(/[^a-zA-Z0-9]/g, '');	
...
//------------------< Bot Owner >-------------------
       ...
       case 'group':
        rows = [
          ...
          section = [{title: "𝐓𝐡𝐚𝐧𝐜𝐤𝐬", rows: rows}]
...
`
 ...
 menumessage = { contentText: `${teks}`, footerText: `𝐙𝐄𝐃 𝐁𝐎𝐓`, buttons: menubutton, headerType: 6, locationMessage: bosco2.message.locationMessage}
 ...
 //------------------< Extra >-------------------
      ...
      case 'tod':
        ...
        case 'play':
               ...
               sendFileFromUrl(res[0].link, audio, {quoted: mek, thumbnail: denis, contextInfo:{externalAdReply: {title: `${songs}`, body: `ᴍᴀᴅᴇ ʙʏ ᴢᴇᴅ ʙᴏᴛ`, mediaType: 2, mediaUrl: 'https://youtu.be/OuYArP4quSA', thumbnail: denis }, mimetype: 'audio/mp4', filename: res[0].output}})
               }
               if (teks.endsWith("-doc")){
               ...
               reply(`.•DOWNLOAD🙊 ${res.all[0].title} •♬•♫•.`)
               let thumbInfo = `
...
➪ 𝐅𝐄𝐀𝐓𝐔𝐑𝐄𝐒 𝐀𝐃𝐃𝐄𝐃 𝐕𝐄𝐒𝐑𝐈𝐎𝐍 𝐂𝐎𝐌𝐌𝐈𝐍𝐆....!`
 ...
 scmessage = { contentText: `${sc}`, footerText: `✪ᴢᴇᴅ ʙᴏᴛ⍟⍟ᴏғᴄ✪`, buttons: scbutton, headerType: 6, locationMessage: sc2.message.locationMessage}
